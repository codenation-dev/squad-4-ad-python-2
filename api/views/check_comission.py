...
def comission_weight_avg(sales: list):
    ...
    :return: Decimal
    """
    comission_values = array(
        [sale["comission_value"] for sale in sales], dtype=dtype(float)
    )
...
def should_notify_user(seller: Seller, amount: Decimal):
    ...
    percentage_cut = sale_percentage(
        sales_comission_value_avg=comissions_weighted_average, percentage=PERCENTAGE
    )
    comissions_weighted_average_minus_percentage_cut = (
        comissions_weighted_average - percentage_cut
    )
    lower_comission_value = (
        comission_value < comissions_weighted_average_minus_percentage_cut
    )
...
def get_seller_last_sales(seller: Seller, max_last_sales: int):
    ...
    :return: list
    """
    last_sales = Sale.objects.filter(seller=seller).order_by("-year", "-month")[
        :max_last_sales
    ]

    sales_ordered_by_comission_value = sorted(
        last_sales, key=lambda x: x.comission_value
    )

    weight_sales_by_comission_value = [
        {"comission_value": sale.comission_value, "weight": weight}
...
def send_email_sale_notification(seller: Seller):
    ...
    context = {"title": subject, "seller_name": seller.name}
