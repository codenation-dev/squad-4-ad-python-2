...
class TestSalesWeightAvg(BaseApiTest):
    ...
    def create_sale(self, seller, year, month, amount):
        return Sale.objects.create(seller=seller, amount=amount, year=year, month=month)
    ...
    def test_const_sales_weight_avg(self):
        ...
        FEBRUARY = 2
        seller = self.get_seller()
        january_sale = self.create_sale(
            seller=seller, month=JANUARY, year=YEAR, amount=4500
        )
        february_sale = self.create_sale(
            seller=seller, month=FEBRUARY, year=YEAR, amount=6700
        )
        january_weight = 1
        february_weight = 2
        weights_sum = january_weight + february_weight
        weighted_avg = (
            (january_sale.comission_value * january_weight)
            + (february_sale.comission_value * february_weight)
        ) / weights_sum
        last_sales = get_seller_last_sales(seller=seller, max_last_sales=MAX_LAST_SALES)
        sale_weighted_avg = comission_weight_avg(sales=last_sales)
        decimal_places = 10

        self.assertAlmostEqual(sale_weighted_avg, weighted_avg, decimal_places)
