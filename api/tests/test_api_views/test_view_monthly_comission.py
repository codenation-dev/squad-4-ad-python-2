...
class TestViewMontlhlyComision(BaseApiTest):
    ...
    def test_monthly_comission(self):
        ...
        self.create_sale(
            seller=first_seller, year=YEAR, month=MONTH, amount=FIRST_SELLER_AMOUNT
        )
        second_seller = self.create_seller(name="Second seller")
        self.create_sale(
            seller=second_seller, year=YEAR, month=MONTH, amount=SECOND_SELLER_AMOUNT
        )
        url = reverse("monthly_comission", kwargs={"month": MONTH})
        request = self.client.get(path=url)
        ...
        self.assertEquals(data[0]["name"], second_seller.name)
        self.assertEquals(data[1]["name"], first_seller.name)
